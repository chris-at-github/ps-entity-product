...
class ProductIndexer extends IndexerBase {
	...
	protected function getElementRecords($record, $foreignField) {
		...
		/** @var QueryBuilder $queryBuilder */
		$queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_foundation_domain_model_elements');
		$statement = $queryBuilder
			->select('*')
	...
	public function customIndexer(array &$indexerConfig, IndexerRunner &$indexerObject): string {
		if($indexerConfig['type'] == ProductIndexer::KEY) {
			...
			while ($record = $statement->fetch()) {
				...
				foreach(['subtitle', 'long_description', 'options', 'technical_data', 'air_consumption_data', 'technical_features_description', 'accesories_description'] as $fieldName) {
